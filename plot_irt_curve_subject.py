...
def plot_3pl_curves(a_list, b_list, beta, question_ids=[1165,34,1410]):     #fixing question ids for comparison
    ...
    plt.xlabel("θ (Mean Subject Ability)")
    plt.ylabel("P (Correct)")
    plt.title("Item Response Curves (3PL Subject Model)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("irt_curve_3pl_subject.png", dpi=300)
    plt.show()

def main():
    # Load saved SubjectIRT model
    with open("subject_irt_model.pkl", "rb") as f:
        model = pickle.load(f)
